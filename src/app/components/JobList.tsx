                    ...
                    {jobs.map((job, index) => (
                        <tr key={index}>
                            {Object.keys(columns).map((key, index) => (
                                <td key={index} className="p-8 border-bottom-light text-center text-title-color font-medium">
                                    { key === "status" || key === "jobType" ? <Badge status={job[key as keyof IJob]} color={job[key as keyof IJob].toLowerCase()} /> 
                                    : key === "roles" ? <div className="text-lg font-[600]">{job[key as keyof IJob]}</div>
                                    : key === "datePosted" ? <div className="text-lg font-[500]">{job[key as keyof IJob]}</div>
                                    : key === "dueDate" ? <div className="text-lg font-[500]">{job[key as keyof IJob]}</div>
                                    : key === "applicants" ? <div className="text-lg font-[500]">{job[key as keyof IJob]}</div>
                                    : key === "needs" ? <div className="flex items-center justify-center gap-1">
                                        <p className="text-title-color font-medium text-lg">{job[key as keyof IJob].split("/")[0]} </p>
                                        <p className="text-not-focus-color font-medium text-lg"> / {job[key as keyof IJob].split("/")[1]}</p>
                                    </div>
